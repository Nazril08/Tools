...
export default function ImageUpscalerPage() {
  ...
  const handleUpscale = async () => {
    ...
    try {
      const response = await fetch("https://api.nzr.web.id/api/image/mosyne/upscale?output=image", {
        method: "POST",
        body: file,
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Gagal meningkatkan resolusi gambar. Silakan coba lagi.")
      }
    ...
    <div className="container mx-auto p-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Tingkatkan resolusi gambar Anda secara otomatis.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            <Button onClick={handleUpscale} disabled={!originalImage || loading}>
              {loading ? "Memproses..." : "Tingkatkan Resolusi"}
            </Button>
            {processedImage && (
              <a href={processedImage} download={`upscaled_${file?.name || "image.png"}`}>
